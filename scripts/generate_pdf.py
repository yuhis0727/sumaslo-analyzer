"""
マルハンM2000蒲田7 Nの日 分析PDF生成（1〜9の日 全対応）
usage: python3 generate_pdf.py          # 1〜9全て
       python3 generate_pdf.py 7        # 7の日のみ
"""
import sys, warnings
...
# ── 全台PDF ───────────────────────────────────────
def make_zentai_pdf(day_num, df_n, event_dates, col_labels, out_path):
    ...
    title = f"{STORE_NAME}　{day_num}の日 台番号別分析"
...
# ── 機種別PDF ─────────────────────────────────────
def make_kishu_pdf(day_num, df_n, event_dates, col_labels, out_path):
    ...
    title  = f"{STORE_NAME}　{day_num}の日 機種別分析"
...
if __name__ == "__main__":
    target_days = [int(sys.argv[1])] if len(sys.argv) > 1 else list(range(1, 10))

    for day_num in target_days:
        days_in_month = [day_num, day_num + 10, day_num + 20]
        df_n = df[df["date"].dt.day.isin(days_in_month)].copy()
        event_dates = sorted(df_n["date"].unique())

        if len(event_dates) == 0:
            print(f"{day_num}の日: データなし スキップ")
            continue

        col_labels = make_col_labels(event_dates)
        print(f"=== {day_num}の日 ({len(event_dates)}日分: {col_labels[0]}〜{col_labels[-1]}) ===")

        out_zentai = f"{STORE_NAME}_{day_num}の日_全台.pdf"
        out_kishu  = f"{STORE_NAME}_{day_num}の日_機種別.pdf"

        make_zentai_pdf(day_num, df_n, event_dates, col_labels, out_zentai)
        make_kishu_pdf(day_num, df_n, event_dates, col_labels, out_kishu)
        print()
