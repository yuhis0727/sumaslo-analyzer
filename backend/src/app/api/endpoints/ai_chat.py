...
===PICKS===
[{"machine_number": 2026, "model_name": "スマスロ甲鉄城のカバネリ海門決戦", "reason": "固定設定6候補"}]
===END_PICKS===
...
def _build_today_context() -> str:
    ...
    try:
        ...
        if summary["judged_entries"] > 0:
            lines.append(f"\n=== 過去の予測実績（直近{summary['judged_entries']}件・的中判定済み） ===")
            ...
            if summary["recent_notes"]:
                ...
                for n in summary["recent_notes"]:
                    hr = f"{n['hit_rate']*100:.0f}%的中" if n["hit_rate"] is not None else "判定待ち"
                    lines.append(f"  {n['date']}（{n.get('tier') or '-'}・{hr}）: {n['note']}")
    except Exception:
        pass
