...
type FixedSix = {
  ...
  diff_over_model: number;
  consecutive_plus: number;
  n_days: number;
};
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
        <div className="p-4 border-b border-gray-100">
          ...
          <span className="text-xs text-gray-400 ml-2">
            {n !== null ? `${n}の日限定` : "全日程"} / 機種平均+1000枚以上 / 連続3回以上プラス
          </span>
        </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-gray-600 text-xs uppercase tracking-wide">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right">機種平均比</th>
                  <th className="px-4 py-3 text-right">連続+</th>
                ...
                {data.map((row, i) => (
                  <tr key={row.machine_number} className="border-t border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right">
                      <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-bold ${winColor(row.win_rate)}`}>
                    ...
                    <td className="px-4 py-3 text-right">
                      <span className="text-green-700 font-semibold">
                        +{row.diff_over_model.toLocaleString()}
                      </span>
                      <span className="text-xs text-gray-400 ml-1">枚</span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <span className={`font-bold ${
                        row.consecutive_plus >= 5 ? "text-green-600" :
                        row.consecutive_plus >= 3 ? "text-yellow-600" : "text-gray-500"
                      }`}>
                        {row.consecutive_plus}連続
                      </span>
                    </td>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800 space-y-1">
        <p className="font-semibold">検出基準:</p>
        <ul className="list-disc list-inside space-y-0.5 text-blue-700">
          <li>同機種の平均差枚を <strong>+1000枚以上</strong> 上回っていること</li>
          <li>直近 <strong>3回以上連続してプラス差枚</strong> であること</li>
          <li>指定した最低回数以上のデータがあること</li>
        </ul>
      </div>
    </div>
  );
